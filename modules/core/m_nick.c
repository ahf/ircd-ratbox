 ...
 *  USA
 *
 *  $Id: m_nick.c 27173 2011-03-28 18:24:39Z moggie $
 */
...
DECLARE_MODULE_AV2(nick, NULL, NULL, nick_clist, NULL, NULL, "$Revision: 27173 $");
...
mc_nick(struct Client *client_p, struct Client *source_p, int parc, const char *parv[])
{
	...
	/* we've got a collision! */
		...
		perform_nickchange_collides(source_p, client_p, target_p, newts, parv[1]);
...
change_local_nick(struct Client *client_p, struct Client *source_p, char *nick, int dosend)
{
	...
	if(dosend)
	{
		...
		source_p->localClient->last_nick_change = rb_time();
		   ...
		   time_t newts, const char *nick, int dosend)
{
	...
	if(!samenick)
	{
		source_p->tsinfo = newts ? newts : rb_time();
		monitor_signoff(source_p);
	}
