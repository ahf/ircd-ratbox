 ...
 *  USA
 *
 *  $Id: epoll.c 26604 2009-06-22 03:28:46Z androsyn $
 */
...
rb_setselect_epoll(rb_fde_t *F, unsigned int type, PF * handler, void *client_data)
{
	...
	{
		rb_lib_log("rb_setselect_epoll(): epoll_ctl failed: %m");
		abort();
	}
...
rb_select_epoll(long delay)
{
	...
	for(i = 0; i < num; i++)
	{
		...
		if(old_flags != flags)
		{
			...
			if(epoll_ctl(ep_info->ep, op, F->fd, &ep_event) != 0)
			{
				rb_lib_log("rb_select_epoll(): epoll_ctl failed: %m");
			}
		}
...
rb_read_timerfd(rb_fde_t *F, void *data)
{
	...
	{
		rb_close(F);
		rb_lib_log("rb_read_timerfd: timerfd[%s] closed on error: %m", event->name);
		return;
	}
